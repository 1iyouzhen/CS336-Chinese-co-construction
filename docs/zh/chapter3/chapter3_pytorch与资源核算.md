...
- 对于 w1：计算 w1.grad 总共需要 2 * B * D * D 次 FLOPs。

总计反向 FLOPs = (2 * B * D * D) + (4 * B * D * K)

说明：第一层这里只计了 $w_1.grad$（`2*B*D*D`）。理论上还能继续算 $x.grad = h_1.grad @ w_1^{\mathsf T}$（同样需要 `2*B*D*D`），但本例输入 $x$ 未设 `requires_grad=True`，PyTorch 会跳过这一步。所以本例反向总计为 `(2*B*D*D) + (4*B*D*K)`，并不恰好是前向的 2 倍。

这并不影响 6× 规则的成立。关键在于：一层完整的反向传播本应包含两个矩阵乘法，分别计算“权重梯度”和“输入梯度”，二者的计算量都等于该层前向；只要该层输入要求梯度，这两个矩阵乘法都会执行，该层反向就是 2× 前向。第二层正是如此：前向 `2*B*D*K`，反向（$w_2.grad + h_1.grad$）共 `4*B*D*K`，恰好 2 倍。第一层因为输入是数据、不求梯度，省掉了“输入梯度”这一步，故只算 1× 前向；但在真实的多层网络中，首层只占总参数的 $O(1/L)$，可忽略，所以整体上“训练一次 ≈ 6 × 数据点数量 × 参数数量”依然成立。
