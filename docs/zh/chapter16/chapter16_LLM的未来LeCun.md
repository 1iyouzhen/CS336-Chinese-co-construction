...
- **训练目标**：   $\| \widehat{s}_{y} - \text{sg}(s_{y}) \|^2$  ，**即预测表征与目标表征的误差，而非像素重建误差**。其中 $sg( \cdot )$ 表示stop-gradient，防止梯度通过 $s_y$ 回传，这是关键技巧——它强制预测器不能"偷懒"依赖解码捷径，必须真正学会从 $s_x$ 推断 $s_y$ 。**请注意JEPA有多种目标函数，这里展示的只是最基础的一种**。
