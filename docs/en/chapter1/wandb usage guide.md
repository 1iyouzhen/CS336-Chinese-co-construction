...
If you are using it for the first time, please refer to the official documentation to [create an account](https://wandb.ai/login?utm_source=github&utm_medium=code&utm_campaign=wandb&utm_content=quickstart) and set up an [API key](https://wandb.ai/settings). Next, I'll walk you through a quick introduction to this tool.
...
> ⚠️ **Note**: Make sure that machine has been logged in via wandb login and that the run IDs have not been deleted.
